...
export default function FacultyPortal({ onBack }) {
  ...
  useEffect(() => { if (loggedIn) fetchResults(); }, [loggedIn]);

  const fetchResults = async () => {
    try {
      const res = await axios.get(`${BACKEND}/votes/results`);
      setResults(res.data);
    } catch (err) { console.log('Error fetching results'); }
  };

  const startBiometric = async (type) => {
    if (!faculty) { setStatus({ msg: '⚠️ Please select your name first', type: 'error' }); return; }
    if (!window.PublicKeyCredential) {
      setStatus({ msg: '❌ Biometric not supported on this device. Please use your phone or a device with fingerprint/face sensor.', type: 'error' });
      return;
    }
    setStatus(null);
    setScanType(type);
    ...
    try {
      ...
      if (!enrolled) { await enrollFingerprint(`faculty_${faculty}`); } else { await verifyFingerprint(`faculty_${faculty}`); }
      setScanning(false);
        ...
        <div className="panel">
          ...
          {!loggedIn && (
            <div className="card">
              ...
              <p style={{fontSize:'0.78rem',color:'#b06080',marginBottom:'14px',textAlign:'center'}}>Verify your identity to access results</p>
              {!scanning ? (
                <>
            ...
            <div className="card" style={{width:'100%',maxWidth:'420px'}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'20px'}}>
                <div>
                  ...
                  <p style={{fontSize:'0.75rem',color:'#b06080'}}>Welcome, Prof. {faculty.charAt(0).toUpperCase() + faculty.slice(1)}</p>
                </div>
                <button className="refresh-btn" onClick={fetchResults}>🔄 Refresh</button>
              </div>
              <div className="stats-row">
                <div className="stat-card"><div className="stat-number">{results.totalVoted}</div><div className="stat-label">Total Votes</div></div>
                <div className="stat-card"><div className="stat-number">250</div><div className="stat-label">Eligible Voters</div></div>
              </div>
              ...
              <p style={{fontSize:'0.75rem',color:'#b06080',textTransform:'uppercase',letterSpacing:'1px',marginBottom:'14px',fontWeight:600}}>Vote Distribution</p>
